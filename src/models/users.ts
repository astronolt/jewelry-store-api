...
export class Users {
   ...
   async index(username: string, password: string): Promise<User | boolean> {
      try {
         ...
         const compareAccess = bcrypt.compareSync(
            password + PEPPER_CRYPT, (userData.password as string)
         )
         if(!compareAccess){
            throw new Error(`Could not log in this user: ${username}. Check your password`)
         }
   ...
   async create(user: User): Promise<User> {
      try {
         ...
         const conn = await client.connect();
         const sql = 'INSERT INTO users (username, password, firstname, lastname ) VALUES($1, $2, $3, $4) RETURNING username, firstname';
