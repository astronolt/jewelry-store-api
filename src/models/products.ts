...
export class Products {
    async index(): Promise<Product[]> {
        try {
            ...
            const sql =
                'SELECT name, description, type, material, price, category, stock FROM products'
    ...
    async show(id: string): Promise<Product> {
        try {
            const conn = await client.connect()
            const sql =
                'SELECT name, description, type, material, price, category, stock FROM products WHERE id=($1)'
            ...
            console.log(error)
            throw new Error(
                `Could not find jewelry with id: ${id}. Error ${error}`
            )
        }
    }
    ...
    async destroy(id: string | number) {
        try {
            ...
            throw new Error(
                `Could not delete jewelry with id: ${id}. Error ${error}`
            )
        }
    }

    async createDummy(): Promise<Product> {
        try {
            let result
            for (const key in PRODUCTDUMMY) {
                result = await this.create(PRODUCTDUMMY[key])
            }
